                ...
                if (UMP_INVALID_SECURE_ID != ump_id) {
                    ...
                    if (NULL != hnd) {
                        *pHandle = hnd;
                        ...
                        ALOGD("%s hnd->format=%d hnd->uoffset=%d hnd->voffset=%d hnd->paddr=%x", __func__, hnd->format, hnd->uoffset, hnd->voffset, hnd->paddr);
                        return 0;
                                            ...
                                            int format, int bpp, int stride)
{
    ...
    ALOGE("%s size=%x usage=%x w=%d h=%d format=%d(%x) bpp=%d stride=%d", __func__, size, usage, w, h, format, format, bpp, stride);
    ...
    /* memset(&(m->info), 1, sizeof(m->info));
    memset(&(m->finfo), 0xA, sizeof(m->finfo));
    m->finfo.id[0] = 'a';
    m->finfo.smem_start = 0xabc;
    m->finfo.smem_len = 0;
     m->finfo.xpanstep = 0x20;
     m->finfo.ypanstep = 3;
     __u16 ywrapstep;
     m->finfo.line_length = 80;
     m->finfo.mmio_start = 445;
     m->finfo.reserved[0] = '3'; */
...
/*  int current = m->framebuffer->base;
    ...
    hnd->height = h;
    hnd->bpp = bpp;
    hnd->paddr = vaddr - m->framebuffer->base;
    hnd->stride = stride;
                              ...
                              int usage, buffer_handle_t* pHandle, int* pStride)
{
    ...
    /*ALOGE("HAL_PIXEL_FORMAT_YCbCr_420_SP %x", HAL_PIXEL_FORMAT_YCbCr_420_SP);
    ALOGE("HAL_PIXEL_FORMAT_YCrCb_420_SP %x", HAL_PIXEL_FORMAT_YCrCb_420_SP);
    ALOGE("HAL_PIXEL_FORMAT_YCbCr_422_SP %x", HAL_PIXEL_FORMAT_YCbCr_422_SP);
    ALOGE("HAL_PIXEL_FORMAT_YCbCr_420_P %x", HAL_PIXEL_FORMAT_YCbCr_420_P);
    ALOGE("HAL_PIXEL_FORMAT_YV12 %x", HAL_PIXEL_FORMAT_YV12);
    ALOGE("HAL_PIXEL_FORMAT_CUSTOM_YCrCb_420_SP %x", HAL_PIXEL_FORMAT_CUSTOM_YCrCb_420_SP);
    ALOGE("HAL_PIXEL_FORMAT_CUSTOM_YCbCr_420_SP_TILED %x", HAL_PIXEL_FORMAT_CUSTOM_YCbCr_420_SP_TILED);
    ALOGE("GGL_PIXEL_FORMAT_L_8 %x", GGL_PIXEL_FORMAT_L_8);
    ALOGE("OMX_COLOR_FormatYUV420Planar %x", OMX_COLOR_FormatYUV420Planar);
    ALOGE("OMX_COLOR_FormatYUV420SemiPlanar %x", OMX_COLOR_FormatYUV420SemiPlanar); */
...
/* TODO: find constants for these literals, it's no problem if they are duplicated */
        ...
        case HAL_PIXEL_FORMAT_YV12: //0x32315659
            ...
            ALOGE("%s added GRALLOC_USAGE_HW_FIMC1", __func__);
        case 0x210:
        case 0x211:
        case 0x212:
        case 0x213:
    	...
    	//ALOGE("%s about to get bpp", __func__);
    ...
    if (usage & GRALLOC_USAGE_HW_FB) {
        err = gralloc_alloc_framebuffer(dev, size, l_usage, pHandle, w, h, format, 32, (int)stride);
    } else
...
static int alloc_device_free(alloc_device_t* dev, buffer_handle_t handle)
{
    ...
    if (hnd->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) {
        ...
        ALOGD("%s hnd->ump_mem_handle=%d hnd->ump_id=%d", __func__, hnd->ump_mem_handle, hnd->ump_id);
#ifdef USE_PARTIAL_FLUSH
