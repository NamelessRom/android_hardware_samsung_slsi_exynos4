                                            ...
                                            int format, int bpp, int stride)
{
    ...
    ALOGE("%s size=%x usage=%x w=%d h=%d format=%d(%x) bpp=%d", __func__, size, usage, w, h, format, format, bpp);
...
/* TODO: find constants for these literals, it's no problem if they are duplicated */
    ...
    if (usage & GRALLOC_USAGE_HW_FB) {
        ...
        err = gralloc_alloc_buffer(dev, size, l_usage, pHandle, w, h, format, bpp, (int)stride_raw, (int)stride);
