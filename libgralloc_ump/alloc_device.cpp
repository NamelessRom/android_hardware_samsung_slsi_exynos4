                                ...
                                int format, int bpp, int stride_raw, int stride)
{
    ...
    if (usage & GRALLOC_USAGE_HW_ION) {
        ...
        if (usage & GRALLOC_USAGE_HW_FIMC1) {
            ALOGE("[XH] usage = GRALLOC_USAGE_HW_FIMC1");

#ifdef SAMSUNG_EXYNOS_CACHE_UMP
            if ((usage&GRALLOC_USAGE_SW_READ_MASK) == GRALLOC_USAGE_SW_READ_OFTEN)
                ump_mem_handle = ump_ref_drv_allocate(size, UMP_REF_DRV_CONSTRAINT_USE_CACHE);
            else
                ump_mem_handle = ump_ref_drv_allocate(size, UMP_REF_DRV_CONSTRAINT_PHYSICALLY_LINEAR);
#else
            ump_mem_handle = ump_ref_drv_allocate(size, UMP_REF_DRV_CONSTRAINT_PHYSICALLY_LINEAR);
#endif
        } else {
        ...
        int ret;
        int offset=0;
        int reserved_mem=0; //TODO - seems to be global
        ...
        if (gReservedMemAddr == 0) {
            ...
            rc = ioctl(fd, VIDIOC_G_FMT, &format);
            if (rc < 0) {
            ...
            reserved_mem = vc.value;

            if (reserved_mem == 0) {
                ALOGE("Error in video VIDIOC_G_CTRL - V4L2_CID_RESERVED_MEM_SIZE=0\n", ret);
                return false;
            }

            if (reserved_mem >= 4096) {
                gReservedMemAddr = gReservedMemAddr + ((reserved_mem - 4096) * 1024);
                reserved_mem = 4096;
            }
        ...
        if ((buffer_offset + size) >= (reserved_mem * 1024))
            buffer_offset = 0;
        ...
        if ( ((usage >= 0) && (usage & (GRALLOC_USAGE_HWC_HWOVERLAY | GRALLOC_USAGE_HW_ION))) ||
            ...
            if (usage & GRALLOC_USAGE_PRIVATE_NONECACHE) {
                //1c08
                private_handle_t* hnd = new private_handle_t(private_handle_t::PRIV_FLAGS_USES_IOCTL,
                                                size, (8192*1024), private_handle_t::LOCK_STATE_MAPPED, 0, 0);
            } else {
                //1bf2
                private_handle_t* hnd = new private_handle_t(private_handle_t::PRIV_FLAGS_NONE_CACHED | private_handle_t::PRIV_FLAGS_USES_IOCTL,
            ...
            //loc_1bb4
            if (usage & GRALLOC_USAGE_PRIVATE_NONECACHE) {
                private_handle_t* hnd = new private_handle_t(private_handle_t::PRIV_FLAGS_USES_HDMI,
                                                size, (8192*1024), private_handle_t::LOCK_STATE_MAPPED, 0, 0);
            } else {
                private_handle_t* hnd = new private_handle_t(private_handle_t::PRIV_FLAGS_NONE_CACHED | private_handle_t::PRIV_FLAGS_USES_HDMI,
                                                size, 0, private_handle_t::LOCK_STATE_MAPPED, 0, 0);
            }
        }
    }
