...
static void *hwc_vsync_thread(void *data)
{
    ...
    FD_ZERO(&exceptfds);
    FD_SET(pdev->vsync_fd, &exceptfds);
    do {
        ssize_t len = read(pdev->vsync_fd, buf, sizeof(buf));
        timestamp = strtoull(buf, NULL, 0);
        if(pdev->procs)
            pdev->procs->vsync(pdev->procs, 0, timestamp);
        select(pdev->vsync_fd + 1, NULL, NULL, &exceptfds, NULL);
        lseek(pdev->vsync_fd, 0, SEEK_SET);
    } while (1);
